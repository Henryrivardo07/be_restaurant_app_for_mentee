...
const restaurantListValidation = [
  ...
  query("priceMin")
    .optional()
    .isInt({ min: 0 })
    .withMessage("Price minimum must be a positive integer"),
  query("priceMax")
    .optional()
    .isInt({ min: 0 })
    .withMessage("Price maximum must be a positive integer"),
  query("rating")
    .optional()
    .isFloat({ min: 0, max: 5 })
...
const restaurantDetailValidation = [
  ...
  query("limitMenu")
    .optional()
    .isInt({ min: 1, max: 50 })
    .withMessage("Menu limit must be between 1 and 50"),
  query("limitReview")
    .optional()
    .isInt({ min: 1, max: 50 })
    .withMessage("Review limit must be between 1 and 50"),
];
    ...
    try {
      ...
      if (priceMin || priceMax) {
        restaurants = restaurants.filter((restaurant) => {
          ...
          if (priceMin)
            matchesPrice =
              matchesPrice && maxPrice >= Number.parseInt(priceMin);
          if (priceMax)
            matchesPrice =
              matchesPrice && minPrice <= Number.parseInt(priceMax);
      ...
      const paginatedRestaurants = formattedRestaurants.slice(
        startIndex,
        endIndex
      );
      ...
      return successResponse(res, {
        ...
        pagination: {
          ...
          totalPages: Math.ceil(
            formattedRestaurants.length / Number.parseInt(limit)
          ),
        },
        filters: {
          location,
    ...
    try {
      ...
      const avgRating =
        restaurant.reviews.length > 0
          ? restaurant.reviews.reduce((sum, review) => sum + review.star, 0) /
